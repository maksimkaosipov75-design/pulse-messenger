...
#[tauri::command]
fn get_current_user(state: tauri::State<std::sync::Arc<StorageService>>) -> Result<Option<User>, String> {
...
#[tauri::command]
fn get_chat(state: tauri::State<std::sync::Arc<StorageService>>, chat_id: String) -> Result<Option<Chat>, String> {
...
fn create_chat(
    ...
    let chat = Chat {
        ...
        is_pinned: false,
        is_muted: false,
        owner_id: if is_group { Some(user.id.clone()) } else { None },
        group_settings: if is_group { Some(GroupSettings::default()) } else { None },
    };

    if is_group {
        group_service.create_group(&chat_id, &user.id, &user.display_name.clone().unwrap_or_else(|| user.username.clone()));
...
#[tauri::command]
fn delete_chat(state: tauri::State<std::sync::Arc<StorageService>>, group_service: tauri::State<GroupService>, chat_id: String) -> Result<(), String> {
...
#[tauri::command]
fn get_group_members(group_service: tauri::State<GroupService>, chat_id: String) -> Vec<GroupMember> {
...
fn send_message(
    ...
    let user = state.get_user_profile()?.ok_or("User profile not created")?;
...
fn send_network_message(
    ...
    let canonical = format!("{}|{}|{}|{}|{}", message_id, chat_id, user.id, content, timestamp_ms);
    ...
    let envelope = MessageEnvelope {
        ...
        sender_name: user.display_name.clone().unwrap_or_else(|| user.username.clone()),
...
fn search_messages(
    ...
    Ok(results.into_iter().map(|(msg, chat_name)| SearchResult {
        message: msg,
        chat_name,
    }).collect())
}
...
#[tauri::command]
fn get_contacts(state: tauri::State<std::sync::Arc<StorageService>>) -> Result<Vec<Contact>, String> {
    state.get_contacts()
}
...
#[tauri::command]
fn remove_contact(state: tauri::State<std::sync::Arc<StorageService>>, user_id: String) -> Result<(), String> {
...
#[tauri::command]
fn update_settings(state: tauri::State<std::sync::Arc<StorageService>>, settings: Settings) -> Result<(), String> {
    state.save_settings(&settings)
}

#[tauri::command]
fn set_theme(state: tauri::State<std::sync::Arc<StorageService>>, theme_id: String) -> Result<(), String> {
...
async fn send_file_message(
...

    {
        let cmd = command_tx.lock().map_err(|e| e.to_string())?;
        if let Some(tx) = cmd.as_ref() {
            tx.send(NetworkCommand::SendMessage { peer_id, data: offer_data })
                .map_err(|e| e.to_string())?;
        } else {
            return Err("Network not started".to_string());
        }
    ...
    for (i, chunk_data) in chunks.iter().enumerate() {
        ...
        let cmd = command_tx.lock().map_err(|e| e.to_string())?;
        if let Some(tx) = cmd.as_ref() {
            tx.send(NetworkCommand::SendMessage { peer_id, data: chunk_bytes })
                .map_err(|e| e.to_string())?;
        }
    }
    ...
    let complete_data = encode_message(&complete)?;
    {
        let cmd = command_tx.lock().map_err(|e| e.to_string())?;
        if let Some(tx) = cmd.as_ref() {
            tx.send(NetworkCommand::SendMessage { peer_id, data: complete_data })
                .map_err(|e| e.to_string())?;
        }
    }

    // Save locally — store the file in our file storage
    let local_path = file_service.save_file(&message_id, &metadata.file_name, &std::fs::read(&file_path).map_err(|e| e.to_string())?)?;
...
fn save_file_to_downloads(
    ...
    let src_path = file_service.get_file_path(&message_id, &file_name)
...
fn get_temp_dir() -> Result<String, String> {
    ...
    dir.to_str().map(|s| s.to_string()).ok_or_else(|| "Failed to get temp dir".to_string())
}
...
fn send_call_offer(
    ...
    let cmd = command_tx.lock().map_err(|e| e.to_string())?;
    if let Some(tx) = cmd.as_ref() {
        tx.send(NetworkCommand::SendMessage { peer_id, data }).map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
fn send_call_answer(
    ...
    let msg = ProtocolMessage::CallAnswer { call_id, caller_id: caller_id.clone(), sdp };
    let data = encode_message(&msg)?;
...
fn send_ice_candidate(
    ...
    let msg = ProtocolMessage::IceCandidate { call_id, candidate, sdp_mid, sdp_m_line_index };
    let data = encode_message(&msg)?;
    let peer_id: libp2p::PeerId = to_peer.parse().map_err(|_| "Invalid peer ID")?;
    let cmd = command_tx.lock().map_err(|e| e.to_string())?;
    if let Some(tx) = cmd.as_ref() {
...
async fn start_network(
    ...
    let (tx, mut event_rx) = services::start_network(peers, is_running, local_peer_id, None).await?;
    let peer_id = {
        let net = network.lock().map_err(|e| e.to_string())?;
        net.get_peer_id()
    };
    ...
    tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            match &event {
                NetworkEvent::MessageReceived { from_peer, data } => {
                    ...
                    match decode_message(data) {
                        Ok(ProtocolMessage::TextMessage(envelope)) => {
                            ...
                            if (now_ms - envelope.timestamp).unsigned_abs() > 5 * 60 * 1000 {
                                log::warn!("Rejected stale message from {} (timestamp drift: {}ms)", envelope.sender_id, (now_ms - envelope.timestamp).unsigned_abs());
                                continue;
                            }
                            ...
                            if !envelope.sender_public_key.is_empty() && !envelope.signature.is_empty() {
                                ...
                                let pk_bytes: [u8; 32] = match envelope.sender_public_key.clone().try_into() {
                                    Ok(b) => b,
                                    Err(_) => {
                                        log::warn!("Invalid public key length from {}, skipping verification", envelope.sender_id);
                                        continue;
                                    }
                                };
                                let sig_bytes: [u8; 64] = match envelope.signature.clone().try_into() {
                                    Ok(b) => b,
                                    Err(_) => {
                                        log::warn!("Invalid signature length from {}, skipping verification", envelope.sender_id);
                                        continue;
                                    }
                                };

                                let canonical = format!("{}|{}|{}|{}|{}",
                                    envelope.message_id, envelope.chat_id, envelope.sender_id,
                                    envelope.content, envelope.timestamp);

                                match VerifyingKey::from_bytes(&pk_bytes) {
                                    Ok(vk) => {
                                        let sig = Signature::from_bytes(&sig_bytes);
                                        if vk.verify(canonical.as_bytes(), &sig).is_err() {
                                            log::warn!("Invalid signature from {}, dropping message", envelope.sender_id);
                                            continue;
                                        }
                                        log::info!("Signature verified for {}", envelope.sender_id);
                                    }
                                    Err(_) => {
                                        log::warn!("Invalid public key from {}, skipping verification", envelope.sender_id);
                                        continue;
                                    }
                                }
                            } else {
                                log::warn!("Missing signature from {}, dropping message", envelope.sender_id);
                                continue;
                            }
                        ...
                        Ok(ProtocolMessage::KeyExchange {
                            ...
                            log::info!(
                                "Key exchange from {} for chat {}",
                                sender_id,
                                chat_id
                            );
                            let _ = app_handle.emit("key-exchange", &serde_json::json!({
                                "chatId": chat_id,
                                "senderId": sender_id,
                        ...
                        Ok(ProtocolMessage::Ack { message_id, status }) => {
                            ...
                            let _ = app_handle.emit("message-ack", &serde_json::json!({
                                "messageId": message_id,
                                "status": format!("{:?}", status),
                            }));
                        }
                        Ok(ProtocolMessage::FileOffer { message_id, chat_id, sender_id, sender_name, file_name, file_size, mime_type, chunk_count, timestamp }) => {
                            log::info!("File offer from {}: {} ({} bytes, {} chunks)", sender_name, file_name, file_size, chunk_count);
                            // Register the incoming transfer
                            if let Some(ft) = app_handle.try_state::<FileTransferService>() {
                                ft.start_incoming(&message_id, FileMetadata {
                            ...
                            let _ = app_handle.emit("file-offer", &serde_json::json!({
                                "messageId": message_id,
                                "chatId": chat_id,
                                "senderId": sender_id,
                        ...
                        Ok(ProtocolMessage::FileChunk { message_id, chunk_index, data }) => {
                            log::info!("File chunk {} for message {} ({} bytes)", chunk_index, message_id, data.len());
                        ...
                        Ok(ProtocolMessage::FileComplete { message_id }) => {
                            ...
                            let _ = app_handle.emit("file-complete", &serde_json::json!({
                                "messageId": message_id,
                            }));
                        }
                        Ok(ProtocolMessage::GroupCreate { chat_id, sender_id, sender_name, group_name, member_ids, timestamp }) => {
                            log::info!("Group '{}' created by {} ({})", group_name, sender_name, sender_id);
                        ...
                        Ok(ProtocolMessage::GroupUpdate { chat_id, sender_id, update_type, timestamp }) => {
                            log::info!("Group update for {}: {:?}", chat_id, update_type);
                            if let Some(gs) = app_handle.try_state::<GroupService>() {
                                match &update_type {
                                    GroupUpdateType::MemberJoined { user_id, display_name } => {
                                    ...
                                    GroupUpdateType::RoleChanged { user_id, new_role, changed_by } => {
                                        let role = match new_role.as_str() {
                                            "admin" => GroupRole::Admin,
                                            _ => GroupRole::Member,
                                        };
                        ...
                        Ok(ProtocolMessage::CallOffer { call_id, chat_id, caller_id, caller_name, callee_id, call_type, sdp, timestamp }) => {
                            log::info!("Call offer from {} ({})", caller_name, call_id);
                            let _ = app_handle.emit("call-offer", &serde_json::json!({
                                "callId": call_id,
                                "chatId": chat_id,
                                "callerId": caller_id,
                        ...
                        Ok(ProtocolMessage::CallAnswer { call_id, caller_id: _, sdp }) => {
                            log::info!("Call answer for {}", call_id);
                            let _ = app_handle.emit("call-answer", &serde_json::json!({
                                "callId": call_id,
                                "sdp": sdp,
                            }));
                        }
                        Ok(ProtocolMessage::IceCandidate { call_id, candidate, sdp_mid, sdp_m_line_index }) => {
                            log::debug!("ICE candidate for {}", call_id);
                            let _ = app_handle.emit("ice-candidate", &serde_json::json!({
                                "callId": call_id,
                                "candidate": candidate,
                                "sdpMid": sdp_mid,
                        ...
                        Ok(ProtocolMessage::CallEnd { call_id, reason }) => {
                            ...
                            let _ = app_handle.emit("call-end", &serde_json::json!({
                                "callId": call_id,
                        ...
                        Ok(ProtocolMessage::CallReject { call_id }) => {
                            ...
                            let _ = app_handle.emit("call-reject", &serde_json::json!({
                                "callId": call_id,
                            }));
                        }
                        Err(e) => {
